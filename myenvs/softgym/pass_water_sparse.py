...
class PassWaterSparseEnv(FluidEnv):
    def __init__(self, observation_mode, action_mode, reward_type =  "sparse", cached_states_path='pass_water_init_states.pkl', **kwargs):
        '''
        ...
        self._goal_save_dir = "save/pass_water/goals/"
        if not osp.exists("save/pass_water/goals/"):
            os.makedirs("save/pass_water/goals/")
    ...
    def _sample_goal(self):
        ...
        # randomize the goal
        # for goal, in_glass should == water_number, and out_glass should == 0
        # goal, terminal_x and in_glass
        self.terminal_x = random.uniform(1,self.max_x)
        ...
        cup_state = np.array([self.terminal_x, self.glass_dis_x, self.glass_dis_z, self.height,
                self._get_current_water_height(), in_glass, out_glass])
        goal = np.hstack([pos, cup_state[0], cup_state[-2]]).flatten()
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        NOTE: the obs and action params are made here to be compatiable with the MultiTask env wrapper.
        """
        _shape = achieved_goal.shape[:-1] + (2,)
        achieved_goal = achieved_goal.reshape(_shape)
        desired_goal = desired_goal.reshape(_shape)

        glass_x = achieved_goal[...,0]
        out_glass = achieved_goal[...,1]

        terminal_x = desired_goal[...,0]
        water_num = desired_goal[...,1]

        reward = -self.water_penalty_coef * (float(out_glass) / water_num)
        reward += -self.distance_coef * np.abs((terminal_x - glass_x))
        if self.reward_type == 'sparse':
            reward = - ((np.abs(terminal_x - glass_x)>=self.dist_thresh).astype(np.float32) + ((out_glass / water_num)>=self.water_thresh).astype(np.float32)).astype(np.bool).astype(np.float32)
            # reward = - (np.abs(terminal_x - glass_x)>=self.dist_thresh or (float(out_glass) / water_num)>=self.water_thresh).astype(np.float32)
    ...
    def reset(self):
        ...
        self.time_step = 0
        obs = self._reset()
        achieved_goal = np.array([obs[0], obs[-2]])
        return {
    ...
    def step(self, action):
        ...
        obs = self._get_obs()
        achived_goal = np.array([obs[0], obs[-2]])
...
if __name__ == '__main__':
    env = PassWaterEnv(observation_mode='cam_rgb',
                         action_mode='direct',
                         render=True,
                         headless=False,
                         horizon=75,
